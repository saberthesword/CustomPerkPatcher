...
namespace CustomPerkCompiler
{
    ...
    {
        // Declare the lazy settings variable
        static Lazy<CustomPerksMapping> _settings = null!;

        public static async Task<int> Main(string[] args)
        {
            return await SynthesisPipeline.Instance
                .AddPatch<ISkyrimMod, ISkyrimModGetter>(RunPatch)
                // Bind the lazy settings generator
                .SetAutogeneratedSettings("Custom Perks Settings", "CustomPerksMapping.json", out _settings)
        ...
        {
            // This instantly loads the file, or generates a default one if it doesn't exist
            CustomPerksMapping customPerks = _settings.Value;

            // Optional: If the generated file is completely empty/null, you can still exit safely
            if (customPerks?.CustomTrees == null || customPerks.CustomTrees.Count == 0)
            {
                Console.WriteLine("[Warning] No custom trees defined in settings. Exiting patcher.");
                return;
            }
